...
URL: `https://associazione-no-profit.demo.federicocalo.dev`
...
2. Aggiungi custom domain in Vercel Dashboard:
   - Settings > Domains > Add `associazione-no-profit.demo.federicocalo.dev`
...
- Output directory: `dist/associazione-no-profit/browser`
...
URL: `https://fedcal.github.io/federico-demo-associazione-no-profit/`
...
- `base: /federico-demo-associazione-no-profit/` (subdirectory GH Pages)
- `cleanUrls: true`
...
Per usare `associazione-no-profit-docs.federicocalo.dev` invece del subdirectory:

1. Cloudflare DNS aggiungi CNAME:
   ```
   associazione-no-profit-docs    CNAME    fedcal.github.io    (proxy OFF)
   ```
2. Repo Settings > Pages > Custom domain: `associazione-no-profit-docs.federicocalo.dev`
